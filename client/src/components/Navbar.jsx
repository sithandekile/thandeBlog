    ...
    <header className="sticky left-0 right-0 top-0 z-20 flex flex-wrap items-center justify-between gap-3 bg-sky-950 p-4 text-white shadow-lg dark:bg-slate-900 dark:text-gray-100">
      ...
      <div className="flex items-center gap-3">
        ...
        <nav
          ...
          } absolute left-0 right-0 top-full bg-sky-950 px-6 pb-4 dark:bg-slate-900 md:static md:block md:bg-transparent md:p-0 md:dark:bg-transparent`}
        >
          <ul className="flex flex-col gap-4 text-sm md:flex-row md:items-center md:gap-4 md:p-4 md:text-base">
            ...
            <li>
              <Link to="/share-story" onClick={closeMenu}>
                Share Your Story
              </Link>
            </li>
            ...
            <li>
              {isSignedIn ? (
                ...
                <Link
                  to="/sign-in"
                  onClick={closeMenu}
                  className="inline-block rounded bg-orange-700 px-3 py-2 font-medium text-white hover:bg-orange-600"
                >
                  Sign in
                </Link>
              )}
